...
export const IOInspectorModal: React.FC<IOInspectorModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal Card */}
      <div className="relative z-10 w-full max-w-3xl bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
        {/* Header */}
        <div className="p-4 sm:p-5 border-b border-slate-800 flex items-center justify-between bg-slate-950/80">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 font-mono">Model: {agent.model} • Latency: {agent.latencyMs}ms</p>
            </div>
          </div>
        ...
        {/* Tab Selector */}
        <div className="flex items-center justify-between px-5 py-2.5 bg-slate-950/40 border-b border-slate-800">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs text-slate-300 bg-slate-800 hover:bg-slate-700 transition-colors font-mono"
          >
        ...
        {/* Code Content View */}
        <div className="p-5 overflow-y-auto font-mono text-xs text-slate-300 bg-slate-950 flex-1 min-h-[300px]">
          {activeTab === 'INPUT' && (
          ...
          {activeTab === 'DIFF' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <div className="text-[11px] font-bold text-slate-400 uppercase mb-2 pb-1 border-b border-slate-800">
                  Input Contract Provided
                </div>
                <pre className="text-emerald-400/90 whitespace-pre-wrap text-[11px]">
                  {inputJson}
                </pre>
              </div>
              <div className="border-t md:border-t-0 md:border-l border-slate-800 pt-4 md:pt-0 md:pl-4">
                ...
                <pre className="text-cyan-400/90 whitespace-pre-wrap text-[11px]">
                  {outputJson}
                </pre>
              </div>
            </div>
