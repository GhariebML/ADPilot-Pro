...
from .agent_events import AgentEventType, AgentLifecycleEvent, event_bus
from .contract_registry import get_agent_contract
...
class BaseAgent(abc.ABC, Generic[InputModel, OutputModel]):
    ...
    def __init__(self) -> None:
        ...
        if self.contract is None:
            self.contract = get_agent_contract(self.name)

    def get_input_schema(self) -> Type[InputModel]:
        """Return the typed Pydantic input model for this agent."""
        return self.input_model

    def get_output_schema(self) -> Type[OutputModel]:
        """Return the typed Pydantic output model for this agent."""
        return self.output_model

    def get_responsibilities(self) -> List[str]:
        """Return the list of runtime responsibilities from the agent's contract."""
        if self.contract:
            return self.contract.responsibilities
        return [f"Execute {self.name} processing"]

    def get_contract(self) -> Optional[AgentContract]:
        """Return the full executable typed AgentContract."""
    ...
    def emit_event(
        ...
        """Emit a structured lifecycle event to the global event bus."""
