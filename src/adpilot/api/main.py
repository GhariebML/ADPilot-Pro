...
# ---------------------------------------------------------------------------
# App initialisation & Lifespan Lifecycle
# ---------------------------------------------------------------------------
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    from ..core.database import init_db, close_db, create_tables, async_session_factory
    await init_db()
    await create_tables()
    ...
    logger.info("AdPilot Application startup complete. Environment: %s", config.environment)
    try:
        yield
    finally:
        logger.info("AdPilot Application shutting down...")
        await scheduler.stop()
        await redis_mgr.close_redis()
        await close_db()
        logger.info("AdPilot Application shutdown complete.")
...
# Attach centralized error handlers and middleware stack
...
async def ready(response: Response) -> Dict[str, Any]:
    """Return 200 when critical dependencies (DB) are ready, or 503 if unavailable."""
    from ..core.health import check_readiness, HealthStatus
