...
class TaskManager:
    ...
    def __init__(self, memory_service: Optional[MemoryService] = None) -> None:
        ...
        self._product_classifier_agent = ProductClassifierAgent()
    ...
    async def run(self, orchestrator_input: OrchestratorInput) -> OrchestratorOutput:
        ...
        context = CampaignContextBuilder.from_brief(
            campaign,
            campaign_id="task-" + _now().replace(":", "").replace(".", "").replace("-", "")[:16],
        )
        await self.memory_service.save_context(context.campaign_id, context)

        # 1. Product Classifier Agent (Phase 3)
        context = await self._run_product_classifier(context)
        ...
        # 3. Strategic Planning & Market Research
    ...
    async def _run_product_classifier(self, context: CampaignContext) -> CampaignContext:
        started = _now()
        record = AgentRunRecord(
            agent_name=self._product_classifier_agent.name,
            status=AgentRunStatus.running,
            started_at=started,
        )
        try:
            context = await self._product_classifier_agent.run(context)
            await self.memory_service.save_context(context.campaign_id, context)
            record.status = AgentRunStatus.success
            if context.classification:
                record.output_snapshot = {
                    "product_type": context.classification.product_type.value,
                    "confidence": context.classification.confidence,
                    "execution_mode": context.classification.recommended_execution_mode.value,
                }
            return context
        except Exception as exc:
            record.status = AgentRunStatus.failed
            record.error_message = str(exc)
            raise AgentExecutionError(f"ProductClassifierAgent failed: {exc}") from exc
        finally:
            record.finished_at = _now()
            self._run_records.append(record)
