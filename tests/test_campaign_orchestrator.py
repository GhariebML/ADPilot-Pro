...
async def test_campaign_orchestrator_retry_and_failure(monkeypatch):
    ...
    # ProductClassifier succeeded, Strategy agent succeeded on 2nd attempt, audience agent failed
    records = orchestrator.agent_run_records
    assert len(records) == 3
    assert records[0].agent_name == "product_classifier_agent"
    assert records[0].status.value == "success"
    assert records[1].agent_name == "strategy_agent"
    assert records[1].status.value == "success"
    assert records[2].agent_name == "audience_agent"
    assert records[2].status.value == "failed"
